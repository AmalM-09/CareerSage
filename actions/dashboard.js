...
export const generateAIInsights = async (industry, subIndustry = "") => {
  const prompt = `
    Analyze the current state of the ${industry} industry${subIndustry ? `, specifically in ${subIndustry}` : ""}, 
...
export async function getIndustryInsights(subIndustry = "") {
  const { userId } = await auth();
  if (!userId) throw new Error("Unauthorized");
  ...
  // If no insights exist, generate them
  if (!user.industryInsight) {
    // const insights = await generateAIInsights(user.industry, subIndustry);
    ...
    const insights = await generateAIInsights(user.industry); // user.industry = "Software, Web"

const industryInsight = await db.industryInsight.create({
  data: {
    industry: user.industry, // "Software, Web"
    ...insights,
